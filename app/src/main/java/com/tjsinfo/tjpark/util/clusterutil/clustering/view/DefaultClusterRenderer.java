...
package com.tjsinfo.tjpark.util.clusterutil.clustering.view;
...
import com.tjsinfo.tjpark.util.clusterutil.MarkerManager;
...
import com.tjsinfo.tjpark.util.clusterutil.projection.Point;
import com.tjsinfo.tjpark.util.clusterutil.projection.SphericalMercatorProjection;
import com.tjsinfo.tjpark.util.clusterutil.ui.IconGenerator;
...
import com.tjsinfo.tjpark.util.clusterutil.clustering.algo.NonHierarchicalDistanceBasedAlgorithm;
import com.tjsinfo.tjpark.util.clusterutil.ui.SquareTextView;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements
    ...
    protected int getBucket(Cluster<T> cluster) {
        ...
        Collection<T> p = cluster.getItems();
        Iterator it =p.iterator();
        int num = 0;
        while(it.hasNext()){
            MapActivity.MyItem myItem = (MapActivity.MyItem)it.next();
            if (myItem.park.getLable().contains("充电")){
                num+=Integer.parseInt(myItem.park.getFast_pile_space_num());
            }
            else  if (myItem.park.getLable().contains("共享")){
                num+=Integer.parseInt(myItem.park.getShare_num());
            }
            else{
                num+=Integer.parseInt(myItem.park.getSpace_num());
            }
        }
            return num;
